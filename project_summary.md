...
当前状态：全部改动已提交并推送，main 分支最新提交 `f2dacf1`，工作区干净。

**下一件待办（与用户讨论）**：基于同类项目调研（设计文档第十二节）调整实现计划。

- 已落地的调整：站内搜索预置（B站/抖音/知乎/淘宝）、记账 CSV 导出（src/utils/csv.ts）、实现计划新增 Task 2.6（内置图标库 + 链接快速过滤）与 Task 7.3（浅色主题与背景）。
- 待与用户讨论：
  1. 是否采纳其余借鉴点（首页网格卡片布局细节、壁纸支持、链接过滤交互、浅色主题的落地时机）；
  2. 确认后更新设计文档与实现计划，再进入 Phase 2；
  3. 是否把"上架 Chrome/Edge 商店"提上日程（影响打包与发布流程）。

**继续方式**：
1. 先读本文件（第七节环境注意事项以实际克隆位置为准，不照搬本机路径）。
2. 再读设计文档第十二节与实现计划末尾"调研调整记录"。
3. 与用户确认调整清单后，更新文档并开始 Phase 2。